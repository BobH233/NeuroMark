#!/usr/bin/env python3
"""Export the official U^2-NetP PyTorch checkpoint to a static ONNX model."""
...
class U2NETPExportWrapper(nn.Module):
...
def _candidate_state_dicts(checkpoint: object) -> Iterable[Dict[str, torch.Tensor]]:
    if isinstance(checkpoint, dict):
        tensor_values = checkpoint.values()
        if checkpoint and all(torch.is_tensor(value) for value in tensor_values):
            yield checkpoint  # raw state_dict
...
def load_weights(model: nn.Module, checkpoint_path: Path) -> None:
    ...
    for candidate in _candidate_state_dicts(checkpoint):
        try:
            model.load_state_dict(_normalize_state_dict(candidate), strict=True)
            return
        except RuntimeError as exc:
            errors.append(str(exc))

    raise RuntimeError(
        "Unable to load checkpoint as an official U2NETP state_dict.\n"
        + "\n".join(errors)
    )
...
def export_to_onnx(
    ...
    wrapper = U2NETPExportWrapper(model).eval()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export official u2netp.pth weights to a static ONNX model for OpenCV DNN."
    )
    parser.add_argument(
        "--weights",
        type=Path,
        required=True,
        help="Path to the official u2netp.pth checkpoint.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("models/u2netp.onnx"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=320,
        help="Static input width/height. OpenCV deployment should keep this at 320.",
    )
    parser.add_argument(
        "--opset",
        type=int,
        default=11,
        help="ONNX opset version. Use 11 or newer for OpenCV cv::dnn compatibility.",
    )
...
def main() -> None:
    ...
    print(f"Exported ONNX model to: {args.output.expanduser().resolve()}")

...
ScanArtifacts scanDocument(cv::dnn::Net& net, const cv::Mat& image) {
    ScanArtifacts artifacts;
    artifacts.saliencyMask = inferSaliencyMask(net, image);
...
void saveDebugOutputs(const cv::Mat& inputImage, const ScanArtifacts& artifacts, const std::string& prefix) {
    ...
    if (!cv::imwrite(prefix + "_mask.png", saliency8u) ||
        !cv::imwrite(prefix + "_binary.png", artifacts.binaryMask) ||
        !cv::imwrite(prefix + "_overlay.png", overlay) ||
        !cv::imwrite(prefix + "_warped.png", artifacts.warpedColor) ||
        !cv::imwrite(prefix + "_scan.png", artifacts.scanned)) {
    ...
    try {
        ...
        if (firstArg == "--benchmark") {
            ...
            const double coldStartMs = measureMilliseconds([&]() {
                cv::dnn::Net coldNet = cv::dnn::readNetFromONNX(modelPath);
                if (coldNet.empty()) {
                    throw std::runtime_error("Failed to load the ONNX model during cold start.");
                }

                coldNet.setPreferableBackend(cv::dnn::DNN_BACKEND_OPENCV);
                coldNet.setPreferableTarget(cv::dnn::DNN_TARGET_CPU);
                ...
                const ScanArtifacts coldArtifacts = scanDocument(coldNet, coldInput);
            ...
            BenchmarkSummary summary =
                benchmarkScanDocument(net, imagePath, outputPath, warmupRuns, measuredRuns);
        ...
        const ScanArtifacts artifacts = scanDocument(net, input);
        if (!cv::imwrite(outputPath, artifacts.scanned)) {
            throw std::runtime_error("Failed to write the output image.");
        }

        if (!debugPrefix.empty()) {
            saveDebugOutputs(input, artifacts, debugPrefix);
        }
